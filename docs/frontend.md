...
**Event Map** (`blocks/event-map/`) — Leaflet needs the client, so this one only partially converts: the wrapper is an Interactivity API island whose `data-wp-context` carries the map config **and** the initial pin payload (`Vandrekalender_Event_Rest_Api::locations_payload()`), so first paint needs no REST call. `view.js` lazy-loads Leaflet from a CDN in `callbacks.init` and only refetches `GET /vandrekalender/v1/events/locations` on `vk:filters-change`. Inside a hidden tab the map has zero size, so it re-measures/re-fits on the Tabs block's `vk:tab-shown` event (dispatched on the activated panel, bubbling) — ResizeObserver alone proved unreliable for the display-none→block flip.
